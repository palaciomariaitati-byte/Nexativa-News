...
const proDashboardCodeWithPayments = `'use client';
...
interface PaymentConfig {
  // Brubank
  brubank_alias: string;
  brubank_cbu: string;
  brubank_titular: string;
  // Banco Nación
  bna_alias: string;
  bna_cbu: string;
  bna_titular: string;
  // Nexora Pay / CUIT
  cuit_titular: string;
  nexorapay_alias: string;
  instrucciones_pago: string;
}

const DEFAULT_PAYMENT_CONFIG: PaymentConfig = {
  brubank_alias: 'NEXORA.BRUBANK',
  brubank_cbu: '1430001713000000000001',
  brubank_titular: 'Nexora Store / Javier López',
  bna_alias: 'NEXORA.NACION',
  bna_cbu: '0110000000000000000002',
  bna_titular: 'Nexora Store / Banco Nación',
  cuit_titular: '20-XXXXXXXX-X',
  nexorapay_alias: 'NEXORAPAY.OFICIAL',
  instrucciones_pago: 'Una vez realizada la transferencia, enviar el comprobante por WhatsApp para la activación inmediata de la licencia.'
};
...
export default function MasterSaaSDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'clientes' | 'cobros_cuentas' | 'actualizaciones' | 'telemetria' | 'qr'>('cobros_cuentas');
  ...
  useEffect(() => {
    try {
      const storedPay = localStorage.getItem('nexora_payment_config');
      if (storedPay) {
        setPaymentConfig(JSON.parse(storedPay));
      }
    } catch {}
  }, []);
  ...
  const savePaymentConfig = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem('nexora_payment_config', JSON.stringify(paymentConfig));
    showToast('¡Cuentas bancarias y Nexora Pay guardados con éxito! 🏦✨');
  };
      ...
      <div className="min-h-screen bg-[#060b13] flex items-center justify-center p-4">
        <div className="bg-[#0f172a] border border-[#e4a834]/40 rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
          ...
          <p className="text-xs text-gray-400 mt-2 mb-6">
            Acceso administrativo confidencial para control de licencias, cuentas de cobro CBU/Alias y telemetría de dispositivos.
          </p>
  ...
  return (
    <div className="min-h-screen bg-[#070d18] text-slate-100 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Licencias, Cobros Brubank / Banco Nación & Nexora Pay</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 bg-white/5 p-1 rounded-2xl border border-white/10">
            <button
              onClick={() => setActiveTab('cobros_cuentas')}
              className={\`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 \${activeTab === 'cobros_cuentas' ? 'bg-[#e4a834] text-black shadow-lg' : 'text-gray-300 hover:text-white'}\`}
            >
              <Wallet className="w-3.5 h-3.5" /> 🏦 Cuentas & Nexora Pay
            </button>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA: CONFIGURACIÓN DE CUENTAS BANCARIAS & NEXORA PAY                  */}
        {/* ========================================================================= */}
        {activeTab === 'cobros_cuentas' && (
          <div>
            <div className="bg-[#0e2430]/90 border border-[#1b4353]/50 rounded-3xl p-6 sm:p-8 mb-8 shadow-2xl">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-xl font-black text-white flex items-center gap-2">
                    <Wallet className="w-6 h-6 text-[#e4a834]" />
                    <span>Configuración de Cuentas para Cobros (Brubank, Banco Nación & Nexora Pay)</span>
                  </h2>
                  <p className="text-xs text-gray-400 mt-1">
                    Estos datos se integran automáticamente en la pasarela de compra de la tienda y en los recordatorios de WhatsApp enviados a tus clientes.
                  </p>
                </div>
                <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 px-3.5 py-1.5 rounded-full text-emerald-400 text-xs font-bold">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                  Cobros Directos Habilitados
                </div>
              </div>

              <form onSubmit={savePaymentConfig} className="space-y-6">
                
                {/* 1. BRUBANK */}
                <div className="bg-[#08121d] border border-purple-500/30 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-purple-500/20 border border-purple-500/40 flex items-center justify-center font-black text-purple-400 text-xs">
                        BRU
                      </div>
                      <div>
                        <h3 className="text-sm font-extrabold text-white">Cuenta Brubank (Digital & Transferencias)</h3>
                        <p className="text-[11px] text-gray-400">Para recibir pagos instantáneos con CBU / Alias</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">Alias Brubank *</label>
                      <input
                        type="text"
                        value={paymentConfig.brubank_alias}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, brubank_alias: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-purple-300 font-mono font-bold focus:outline-none focus:border-purple-400"
                        placeholder="Ej: NEXORA.BRUBANK"
                        required
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">CBU / CVU Brubank *</label>
                      <input
                        type="text"
                        value={paymentConfig.brubank_cbu}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, brubank_cbu: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-white font-mono focus:outline-none focus:border-purple-400"
                        placeholder="22 dígitos..."
                        required
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">Titular de Cuenta *</label>
                      <input
                        type="text"
                        value={paymentConfig.brubank_titular}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, brubank_titular: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-purple-400"
                ...
                {/* 2. BANCO NACIÓN */}
                <div className="bg-[#08121d] border border-cyan-500/30 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-cyan-500/20 border border-cyan-500/40 flex items-center justify-center font-black text-cyan-400 text-xs">
                        BNA
                      </div>
                      <div>
                        <h3 className="text-sm font-extrabold text-white">Cuenta Banco de la Nación Argentina (BNA)</h3>
                        <p className="text-[11px] text-gray-400">Para clientes que prefieren transferencias tradicionales o cheques</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">Alias Banco Nación *</label>
                      <input
                        type="text"
                        value={paymentConfig.bna_alias}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, bna_alias: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-cyan-300 font-mono font-bold focus:outline-none focus:border-cyan-400"
                        placeholder="Ej: NEXORA.NACION"
                        required
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">CBU Banco Nación *</label>
                      <input
                        type="text"
                        value={paymentConfig.bna_cbu}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, bna_cbu: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-white font-mono focus:outline-none focus:border-cyan-400"
                        placeholder="22 dígitos..."
                        required
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">Titular BNA *</label>
                      <input
                        type="text"
                        value={paymentConfig.bna_titular}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, bna_titular: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-cyan-400"
                        placeholder="Nombre y Apellido..."
                        required
                      />
                    </div>
                ...
                {/* 3. NEXORA PAY & IDENTIFICACIÓN FISCAL */}
                <div className="bg-[#08121d] border border-[#e4a834]/30 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-[#e4a834]/20 border border-[#e4a834]/40 flex items-center justify-center font-black text-[#e4a834] text-xs">
                        PAY
                      </div>
                      <div>
                        <h3 className="text-sm font-extrabold text-white">Nexora Pay & CUIT Fiscal</h3>
                        <p className="text-[11px] text-gray-400">Identificador unificado de facturación y cobro</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">Alias Unificado Nexora Pay</label>
                      <input
                        type="text"
                        value={paymentConfig.nexorapay_alias}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, nexorapay_alias: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-[#e4a834] font-mono font-bold focus:outline-none focus:border-[#e4a834]"
                        placeholder="Ej: NEXORAPAY.OFICIAL"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-300 block mb-1">CUIT / CUIL del Titular</label>
                      <input
                        type="text"
                        value={paymentConfig.cuit_titular}
                        onChange={(e) => setPaymentConfig({ ...paymentConfig, cuit_titular: e.target.value })}
                        className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-white font-mono focus:outline-none focus:border-[#e4a834]"
                  ...
                  <div className="mt-4">
                    <label className="text-xs font-bold text-gray-300 block mb-1">Instrucciones de Activación tras Pago</label>
                    <textarea
                      value={paymentConfig.instrucciones_pago}
                      onChange={(e) => setPaymentConfig({ ...paymentConfig, instrucciones_pago: e.target.value })}
                      rows={2}
                      className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2 text-xs text-gray-200 focus:outline-none focus:border-[#e4a834]"
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-end pt-2">
                  <button
                    type="submit"
                    className="px-8 py-3.5 rounded-2xl bg-[#e4a834] hover:bg-[#cf9323] text-black font-extrabold text-xs uppercase tracking-wider transition-all shadow-xl shadow-[#e4a834]/20 flex items-center gap-2 cursor-pointer"
                  >
                    <Save className="w-4 h-4" /> Guardar Cuentas de Cobro
                  </button>
                </div>
            ...
            <div className="bg-[#0e2430]/60 border border-[#1b4353]/30 rounded-2xl p-6">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                <span>👁️ Vista Previa del Módulo de Pago para el Cliente:</span>
              </h3>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                
                {/* Card Brubank */}
                <div className="bg-black/40 border border-purple-500/20 rounded-xl p-4 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-extrabold text-sm text-purple-300">Brubank (Transferencia)</span>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-purple-500/20 text-purple-300">0% Comisión</span>
                    </div>
                    <div className="text-xs text-gray-300">Alias: <strong className="text-white font-mono">{paymentConfig.brubank_alias}</strong></div>
                    <div className="text-xs text-gray-400 font-mono mt-1 text-[11px] truncate">CBU: {paymentConfig.brubank_cbu}</div>
                    <div className="text-xs text-gray-400 mt-1">Titular: {paymentConfig.brubank_titular}</div>
                  </div>
                  <button
                    onClick={() => copyToClipboard(paymentConfig.brubank_alias, 'bru_alias')}
                    className="mt-3 w-full py-2 bg-purple-500/15 hover:bg-purple-500/30 text-purple-300 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                  >
                    {copiedKey === 'bru_alias' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    Copiar Alias Brubank
                  </button>
                </div>

                {/* Card Banco Nación */}
                <div className="bg-black/40 border border-cyan-500/20 rounded-xl p-4 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-extrabold text-sm text-cyan-300">Banco Nación (BNA)</span>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-cyan-500/20 text-cyan-300">Oficial</span>
                    </div>
                    <div className="text-xs text-gray-300">Alias: <strong className="text-white font-mono">{paymentConfig.bna_alias}</strong></div>
                    <div className="text-xs text-gray-400 font-mono mt-1 text-[11px] truncate">CBU: {paymentConfig.bna_cbu}</div>
                    <div className="text-xs text-gray-400 mt-1">Titular: {paymentConfig.bna_titular}</div>
                  </div>
                  <button
                    onClick={() => copyToClipboard(paymentConfig.bna_alias, 'bna_alias')}
                    className="mt-3 w-full py-2 bg-cyan-500/15 hover:bg-cyan-500/30 text-cyan-300 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                  >
                    {copiedKey === 'bna_alias' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    Copiar Alias Banco Nación
                  </button>
                </div>
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA: CLIENTES, LICENCIAS Y RÉGIMEN DE COBROS                        */}
        {/* ========================================================================= */}
        {activeTab === 'clientes' && (
          <div>
                  ...
                  <tbody className="divide-y divide-[#1b4353]/30">
                    {clients.map(c => {
                      ...
                      const waPaymentMsg = encodeURIComponent(
                        \`Hola \${c.nombre_comercio}, te saludamos desde Nexora Store.\\n\\nTe recordamos el estado de tu licencia de \${c.sistema_nombre} con vencimiento el \${c.licencia_vencimiento} (Monto: USD \$\${c.abono_usd} / ~$\${(c.abono_usd * 1550).toLocaleString('es-AR')} ARS).\\n\\n🏦 Datos para abonar por Transferencia:\\n• Brubank Alias: \${paymentConfig.brubank_alias}\\n• Banco Nación Alias: \${paymentConfig.bna_alias}\\n• Nexora Pay: \${paymentConfig.nexorapay_alias}\\n\\n¡Avísanos enviando el comprobante por acá para renovar tu licencia!\`
                      );
                        ...
                        <tr key={c.id_cliente} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex items-center justify-end space-x-2">
                              {/* Botón WhatsApp Cobro con cuentas */}
                              <a
                                href={\`https://api.whatsapp.com/send?phone=\${c.telefono_whatsapp}&text=\${waPaymentMsg}\`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="p-2 rounded-xl bg-[#25D366]/20 hover:bg-[#25D366] text-[#25D366] hover:text-black font-bold transition-all flex items-center gap-1"
                                title="Enviar Aviso de Cobro con CBU/Alias por WhatsApp"
                              >
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA: ACTUALIZACIONES DE SOFTWARE OTA & CONTROL DE VERSIONES          */}
        {/* ========================================================================= */}
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA: TELEMETRÍA Y CONTROL DE TERMINALES (DEVICE WATCHDOG)           */}
        {/* ========================================================================= */}
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA: DIFUSIÓN Y GENERADOR DE CÓDIGOS QR                             */}
        {/* ========================================================================= */}
        {activeTab === 'qr' && (
          <div>
...
fs.writeFileSync('D:/BARES 2026/Nexora_Store/app/dashboard/page.tsx', proDashboardCodeWithPayments, 'utf-8');
console.log('Successfully written Master SaaS Dashboard page.tsx with Brubank, Banco Nación & Nexora Pay!');
