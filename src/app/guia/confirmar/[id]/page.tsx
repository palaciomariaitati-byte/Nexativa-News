...
import { CheckCircle2, ShieldCheck, Building2, MapPin, Phone, Mail, Globe, ArrowRight, Sparkles, ExternalLink } from "lucide-react";
    ...
    async function loadBusiness() {
      ...
      try {
        ...
        if (data.success && data.business) {
          ...
          // Fallback visual si el ID es de demostración
          setBusiness({
            id: businessId,
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8 border-b border-slate-800 pb-6">
          ...
          <h1 className="text-3xl font-black text-white tracking-tight mb-2">
            Confirmación de Ficha Comercial
          </h1>
          <p className="text-sm text-slate-400 max-w-lg mx-auto">
            Revisá los datos de tu empresa antes de ser publicada oficialmente en las Páginas Amarillas de la región.
          </p>
        </div>
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 mb-6 leading-relaxed text-sm text-slate-300 shadow-xl">
          <h2 className="text-base font-bold text-white mb-2 flex items-center gap-2">
            <span>🏛️</span>
            <span>Estimado referente de {business?.name}:</span>
          </h2>
          <p className="mb-3">
            Desde la Redacción y Equipo de Desarrollo de <strong>Nexativa News</strong> (nexativanews.com.ar), hemos seleccionado a tu empresa para otorgarle una <strong>Ficha Digital de Cortesía 100% Gratuita</strong> en nuestro portal de cercanía.
          </p>
          <p className="text-xs text-slate-400 bg-slate-950/80 p-3 rounded-xl border border-slate-800/80">
            🔒 <strong>Sin costo ni comisiones:</strong> Tu ficha incluye enlace directo a tu WhatsApp de ventas, ubicación geolocalizada y respaldo periodístico comunitario.
          </p>
        </div>

        {/* Tarjeta Vista Previa del Comercio */}
        {business && (
          <div className="bg-slate-900 border border-cyan-500/30 rounded-2xl p-6 mb-8 shadow-2xl relative overflow-hidden">
            <div className="flex items-start justify-between gap-4 mb-4 border-b border-slate-800 pb-4">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white">{business.name}</h3>
              </div>
            ...
            <div className="space-y-2 text-xs text-slate-300 mb-6">
              <div className="flex items-center gap-2">
                ...
                <span>{business.address || "Ituzaingó, Corrientes"}</span>
              </div>
              {business.whatsapp && (
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4 text-emerald-400" />
                  <span>WhatsApp de Ventas: +{business.whatsapp}</span>
                </div>
              )}
              {business.email && (
                <div className="flex items-center gap-2">
            ...
            {activated ? (
              <div className="bg-emerald-950/80 border border-emerald-500/40 rounded-xl p-5 text-center text-emerald-200 space-y-3">
                ...
                <p className="text-xs text-emerald-300">
                  Tu comercio ya se encuentra activo públicamente en el portal de Páginas Amarillas.
                </p>
                <div className="pt-2">
                  <Link
                    href="/guia"
