...
import { Home, Newspaper, Store, PlayCircle, Palette } from "lucide-react";
...
export default function MobileBottomNav() {
  ...
  return (
    <div className="sm:hidden fixed bottom-0 left-0 right-0 z-50 glass-panel border-t border-white/10 flex justify-around items-center h-16 pb-safe px-1">
      <a href="/" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname === "/" ? "text-[var(--color-brand-accent)]" : "text-gray-400 hover:text-white transition-colors"}`}>
      ...
      <Link href="/cultura" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname === "/cultura" ? "text-[var(--color-brand-accent)]" : "text-gray-400 hover:text-white transition-colors"}`}>
        <Palette className="w-5 h-5" />
        <span className="text-[10px] font-bold uppercase tracking-wider">Cultura</span>
      </Link>
      <button onClick={scrollToStream} className="flex flex-col items-center justify-center w-full h-full space-y-1 text-red-500 hover:text-red-400 transition-colors">
        ...
        <span className="text-[10px] font-bold uppercase tracking-wider">Vivo</span>
      </button>
